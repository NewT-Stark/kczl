...
<html>
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card m-b-30">
                            <div class="card-body">
                                <h4 class="mt-0 header-title" style="margin-left:10px;">审核学生成绩</h4>
                                <table class="table table-striped table-hover">
                                    ...
                                    <tbody>
                                    <c:forEach items="${admsco}" var="sco">
                                        <c:if test="${sco.status eq '已送审' }">
	                                        <tr>
	                                            ...
	                                            <td>${sco.status}</td>
	                                            <td>
	                                                <a class="myButton" target="_self" onclick="toUpdateSco2(${sco.student_id}, ${sco.course_id })" style="width:45px;cursor: pointer">通过</a>
	                                            	<a class="myButton1" target="_self" onclick="toUpdateSco3(${sco.student_id}, ${sco.course_id })" style="width:45px;cursor: pointer">驳回</a>
	                                            </td>
	                                        </tr>
                                        </c:if>
                                    </c:forEach>
                                ...
                                <div class="row">
                                    <div style="display: flex;flex-direction: row-reverse;" class="col-xs-3 col-lg-offset-8">
                                        <button type="button" class="myButton" onclick="as2()">一键通过</button><br>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        as2 = function() {
            $.ajax({
                type:"post",
                url:"http://localhost:8080/kczl/assentsco1",
        ...
        toUpdateSco2 = function(id, cid) {
            $.ajax({
                type:"post",
                url:"http://localhost:8080/kczl/admintosentsco?id="+id+"&cid="+cid,
                success:function(msg){
                    $("#main").html(msg);
                },
                error:function(){
        ...
        toUpdateSco3 = function(id, cid) {
            $.ajax({
                type:"post",
                url:"http://localhost:8080/kczl/admindisco?id="+id+"&cid="+cid,
                success:function(msg){
                    $("#main").html(msg);
                },
                error:function(){
